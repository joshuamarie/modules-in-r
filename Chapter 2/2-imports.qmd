...
The infix operators like `%>%` from `{magrittr}` are considered [non-syntactic names](https://adv-r.hadley.nz/names-values.html#non-syntactic). In R, operators and functions that use special characters (like `+`, `*`, `%>%`, `%in%`, `%*%`, etc.) are called *infix operators* but internally parsed as the usual function call in R, e.g. `` `+`(a, b) ``. These operators require special handling when importing with `{box}`. Regardless, yes, these are still *functions*.
